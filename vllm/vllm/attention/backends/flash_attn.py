...
class FlashAttentionBackend(AttentionBackend):
    ...
    def get_kv_cache_shape(
        ...
        return (4, num_blocks // 2, block_size, num_kv_heads, head_size)
...
class FlashAttentionImpl(AttentionImpl):
    ...
    def forward(
        ...
        if kv_cache.numel() > 0:
            key_cache = kv_cache[0] if self.aligned else kv_cache[2] 
            value_cache = kv_cache[1] if self.aligned else kv_cache[3] 
            ...
            if (attn_type != AttentionType.ENCODER) and (key is not None) and (
                ...
                torch.ops._C_cache_ops.reshape_and_cache_flash(
                    key,
                    value,
                    kv_cache[0] if self.aligned else kv_cache[2],
                    kv_cache[1] if self.aligned else kv_cache[3],
