...
class CustomDPOTrainer(DPOTrainer):
    def __init__(
        ...
        aligned_model: Optional[Union["PreTrainedModel", torch.nn.Module]],
        ...
        if ref_model is not None:
            if self.is_deepspeed_enabled:
                if not (
                    ...
                    self.aligned_model = self._prepare_deepspeed(self.aligned_model)
            else:
                self.ref_model = self.accelerator.prepare_model(self.ref_model, evaluation_mode=True)
                self.ref_model.eval()
                
                self.aligned_model = self.accelerator.prepare_model(self.aligned_model, evaluation_mode=True)
    ...
    def compute_preference_loss_1(
        self,
        policy_chosen_logits: "torch.Tensor",
        reference_chosen_logits: "torch.Tensor",
        aligned_chosen_logits: Optional["torch.Tensor"],
    ) -> tuple["torch.Tensor", "torch.Tensor", "torch.Tensor"]:
        r"""
        Computes loss for preference learning.
        """

        target_logits = self.beta * aligned_chosen_logits  +  (1 - self.beta) * reference_chosen_logits
                
        T = 1.0  
        kl_loss_fn = torch.nn.KLDivLoss(reduction="batchmean")
        log_p_student = F.log_softmax(policy_chosen_logits / T, dim=1)
        p_teacher = F.softmax(target_logits / T, dim=1)
        
        losses = kl_loss_fn(log_p_student[0], p_teacher[0]) * (T * T) 
        
        return losses, 0.0, None
    ...
    def get_batch_loss_metrics(
        ...
        reference_chosen_logps, reference_rejected_logps, reference_chosen_logits, reference_rejected_logits = self.compute_reference_log_probs(model, batch)
        
        ref_context = nullcontext()
        with torch.no_grad(), ref_context:
            aligned_chosen_logps, aligned_rejected_logps, aligned_chosen_logits, aligned_rejected_logits, *_ = self.concatenated_forward(self.aligned_model, batch)
        
        
        losses, _, _ = self.compute_preference_loss_1(
            policy_chosen_logits,
            reference_chosen_logits,
            aligned_chosen_logits,
        )
